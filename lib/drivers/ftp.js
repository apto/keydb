...
var createFTPSource = function (next, options) {
  ...
  var ftp = {
    ...
    size : q.nbind(_ftp.raw.size, _ftp.raw)
  };
  ...
  var ops = {
    get: function (msg) {
      ...
      return ftp.ls(msg.key).then(function (res) {
        // ftp size can be done on a file only. Other wise it fails.
        return ftp.size(msg.key).then(function (size) {
          // if size is greater than zero then its a file
