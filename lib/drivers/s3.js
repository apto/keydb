  ...
  var ops = {
    get: function (msg) {
      ...
      return s3.headObject({
        ...
        if(err.statusCode === 404) {
          ...
          return folderContents(msg.key, false).then(function (results) {
    ...
    meta: function (msg) {
      ...
      return s3.headObject({
        ...
        var responseMsg = { key: msg.key };
        if(data.ContentLength) { responseMsg.size = data.ContentLength; }
        if(data.ETag) { responseMsg.etag = data.ETag; }
        if(data.LastModified) { responseMsg.lastModified = data.LastModified; }
        return responseMsg;
      }, 
      function (err) {
        // not a file object: the key is a directory (or an invalid key)
      ...
      if (data.Contents) {
        data.Contents.forEach(function (file) {
          if (includeFolder || file.Key !== params.Prefix) {
            var fileMeta = { key: normalize(file.Key, params.Prefix) };
            if(file.ContentLength) { fileMeta.size = file.size; }
            if(file.ETag) { fileMeta.etag = file.ETag; }
            if(file.LastModified) { fileMeta.lastModified = file.LastModified; }
