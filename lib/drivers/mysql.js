...
var createMySqlSource = function (next, options) {
      ...
      return db.createTable(tableName, defs)
        .then(function () {
          if (table.createdTime) {
            // This is just for mysql 5.5, which is being used in Travis. 5.6
            // allows multiple current timestamps.
            return db.ensureColumn(tableName, table.createdTime + ' TIMESTAMP DEFAULT 0');
          }
        })
        .then(function () {
  ...
  var ops = {
    ...
    create: function (msg) {
      var table = options.tables[msg.table];
      var columnNames = Object.keys(msg.attributes).join(',');
      if (table.createdTime) {
        columnNames += ',' + table.createdTime;
      }
      var columnValues = Object.keys(msg.attributes).map(function (key) {
